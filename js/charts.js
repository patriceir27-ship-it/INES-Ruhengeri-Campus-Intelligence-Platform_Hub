...
window.INES_CHARTS = {
  ...
  renderEnergyChartData, renderResearchBarLive,
};

/* Live energy chart from DB readings */
function renderEnergyChartData(id, readings) {
  ...
  new Chart(el, {
    ...
    options: {
      ...
      plugins: { legend: { display: false } },
      scales: {
        x: { ticks: { color:'#94A3B8' }, grid: { color:'#1E3A57' } },
        y: { ticks: { color:'#94A3B8' }, grid: { color:'#1E3A57' } }
      }
    }
  });
}

/* Research bar from live dept data */
function renderResearchBarLive(id, deptData) {
  ...
  new Chart(el, {
    type: 'bar',
    data: {
      labels: deptData.map(d => d.dept||d.code),
      datasets: [{ label:'Papers', data: deptData.map(d => parseInt(d.papers||0)),
        backgroundColor:['#7C3AED','#EF4444','#00C2FF','#10B981','#F59E0B','#3B82F6','#64748B','#EC4899'] }]
    },
    options: {
      responsive:true, maintainAspectRatio:false,
      plugins:{ legend:{display:false} },
