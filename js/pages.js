...
const { KPI, ATTENDANCE_WEEK, VISITOR_HOURLY, DEPT_PERF, GRADE_DIST,
        AT_RISK, STAFF_LIST, VISITOR_LOG, COURSES, INCIDENTS,
        AI_MODELS, FINANCE, RESEARCH, FACILITIES, HRM, ORG } = window.INES;
...
<div class="chart-card" style="margin-bottom:14px">
  <div class="chart-title"><div class="chart-title-left">📚 Course Performance (${AI_MODELS.courseDifficulty.name})</div></div>
  <div class="table-wrap">
    <table class="data-table">
      <tr><th>Course</th><th>Dept</th><th>Students</th><th>Pass Rate</th><th>Avg Score</th><th>Difficulty</th></tr>
      ${COURSES.map(c=>`<tr>
...
PAGES.predictions = () => `
<div class="ai-card">
  <div class="ai-header"><div class="ai-pulse"></div><div class="ai-label">AI PREDICTION ENGINE — Active Models: v2.4.1</div><span class="badge badge-purple">8 Models Active</span></div>
  <div class="ai-text">All models will be trained and uploaded to GitHub. Model files will be loaded from <code style="color:#00C2FF;font-size:11px">models/</code> directory. Last dataset sync: April 5, 2026 · Processing 2.1M+ data points.</div>
</div>

<div class="section-title">🎓 Enrollment & Academic</div>
<div class="chart-grid-3">
  ${Object.values(AI_MODELS).slice(0,3).map(m=>`
  <div class="pred-card">
    <div class="pred-header"><div style="font-size:13px;font-weight:700;color:#F0F6FF">${m.description.split(' ').slice(0,4).join(' ')}</div><span class="badge badge-purple">AI</span></div>
    <div class="pred-value" style="color:#00C2FF">${m.prediction.value}</div>
    <div class="pred-desc">${m.prediction.label}</div>
    <div class="confidence">Model: <code style="font-size:10px;color:#7C3AED">${m.name}.json</code></div>
    <div class="confidence">Confidence: ${m.prediction.confidence}%</div>
    <div class="conf-bar"><div class="conf-fill" style="width:${m.prediction.confidence}%"></div></div>
  </div>`).join('')}
</div>

<div class="section-title">👥 Staff & Operations</div>
<div class="chart-grid-3">
  ${Object.values(AI_MODELS).slice(3,6).map(m=>`
  <div class="pred-card">
    <div class="pred-header"><div style="font-size:13px;font-weight:700;color:#F0F6FF">${m.description.split(' ').slice(0,4).join(' ')}</div><span class="badge badge-blue">AI</span></div>
    <div class="pred-value" style="color:#F59E0B">${m.prediction.value}</div>
    <div class="pred-desc">${m.prediction.label}</div>
    <div class="confidence">Model: <code style="font-size:10px;color:#7C3AED">${m.name}.json</code></div>
    <div class="confidence">Confidence: ${m.prediction.confidence}%</div>
    <div class="conf-bar"><div class="conf-fill" style="width:${m.prediction.confidence}%;background:#F59E0B"></div></div>
  </div>`).join('')}
</div>

<div class="section-title">⚡ Infrastructure & Energy</div>
<div class="chart-grid-3">
  ${Object.values(AI_MODELS).slice(6).map(m=>`
  <div class="pred-card">
    <div class="pred-header"><div style="font-size:13px;font-weight:700;color:#F0F6FF">${m.description.split(' ').slice(0,4).join(' ')}</div><span class="badge badge-amber">AI</span></div>
    <div class="pred-value" style="color:#10B981">${m.prediction.value}</div>
    <div class="pred-desc">${m.prediction.label}</div>
    <div class="confidence">Model: <code style="font-size:10px;color:#7C3AED">${m.name}.json</code></div>
    <div class="confidence">Confidence: ${m.prediction.confidence}%</div>
    <div class="conf-bar"><div class="conf-fill" style="width:${m.prediction.confidence}%;background:#10B981"></div></div>
  </div>`).join('')}
</div>

<div class="chart-card">
  <div class="chart-title"><div class="chart-title-left">📊 Enrollment Trend + AI Forecast (${AI_MODELS.enrollmentForecast.name})</div></div>
